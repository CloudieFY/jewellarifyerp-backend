...
router.post('/login', async (req: Request, res: Response) => {
  try {
    ...
    if (shopSlug && String(shopSlug).trim()) {
      ...
      const user = await tenantModels.User.findOne({ username: cleanUsername });
      ...
      // Search across active shops (e.g. Karigar direct login without shop ID)
      const activeShops = await Shop.find({ status: { $ne: 'suspended' } });
      for (const shop of activeShops) {
        if (shop.subscriptionEndDate && shop.subscriptionEndDate < new Date()) continue;
        try {
          const tenantModels = await getTenantContext(shop.dbName);
          const user = await tenantModels.User.findOne({ username: cleanUsername });
          if (user && user.isActive) {
          ...
          // ignore individual shop context lookup failure
        }
      }
    }
    ...
    const token = signTenantToken({
      ...
      karigarRefId: targetUser.karigarRefId,
    });

    res.json({
      token,
      user: targetUser.toJSON(),
      shop: targetShop.toJSON(),
    });
  } catch (error: any) {
...
// Set own UI language preference (self-service, any role). Persisted on the
// User doc so it's restored automatically on next login, from any device.
